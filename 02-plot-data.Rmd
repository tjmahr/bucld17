...
binned <- looks %>% 
  ...
  rename(Time = BinTime) %>% 
  mutate(Cond2 = factor(Condition, c("real", "MP", "nonsense"),
                        labels = c("Real word", "Mispronunciation", "Nonword")),
         Group2 = factor(Group, c("CochlearImplant", "NormalHearing"), 
                         labels = c("Cochlear implant", "Normal hearing")))

ggplot(binned) + 
  aes(x = Time, y = Prop, color = Group2) + 
  hline_chance() +
  vline_onset() +
  stat_mean_se() + 
  facet_wrap("Cond2") + 
